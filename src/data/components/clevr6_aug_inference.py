...
class CLEVR6AugInference(Dataset):
    def __init__(
        self,
    ...
    def __getitem__(self, index):
        ...
        if not self.train:
            ...
            bbox_ori = list()
            center_ori = list()
            for obj_idx, mask_filename in enumerate(self.masks[filename]):
                mask = (
                ...
                if obj_idx > 0: # exclude bakcground 		
                    m = masks_ori[-1][0] # squeeze(0)
                    nz = torch.nonzero(m) # nz: (n, 2) -> (y, x) coord of non-zero values	

                    if nz.nelement() > 0: # exclude objs removed by transform
                        y_start = torch.min(nz[:, 0])
                        y_end = torch.max(nz[:, 0])
                        x_start = torch.min(nz[:, 1])
                        x_end = torch.max(nz[:, 1])
                    
                        bbox_ori.append(torch.tensor([x_start, x_end, y_start, y_end]))
                        center_ori.append(torch.tensor([(x_start + x_end) / 2, (y_start + y_end) / 2]))
                    
                        ## sanity check
                        # sample["img_ori"][:, y_start:y_end, x_start:x_end] = 0 
	
            bbox_ori = torch.stack(bbox_ori, dim=0) / self.img_size
            center_ori = torch.stack(center_ori, dim=0) / self.img_size
            sample["bbox_ori"] = bbox_ori
            sample["center_ori"] = center_ori
